...
module PosMap = Map.Make (struct t = int * int let compare p1 p2 =
    compare (pos_sym_encode p1) (pos_sym_encode p2)
    end)
let pos_val_map =
...
let opening_map =
    ...
    in
    List.fold_left (fun map moves_array ->
        let (m, b, c)  = List.fold_left (fun (map, board) pos ->
            if is_valid_move board pos then
                let bb = minimax_move board pos in
                opening_map_add board bb map, bb
            else
                map, board
            ) (board_init ()) moves_array
        in
            m
...
(*******************BOARD*******************)
(*move by black*)
let board_move_black ((bb, bw) : board_t) pos = (bit_set bb (pos_encode pos), bw)

let board_move_white (bb, bw) pos = (bb, bit_set bw (pos_encode pos))
...
let board_rotate_one board =
    List.fold_left (fun bb pos -> board_move_black bb (pos_rotate_one pos) (board_get_color board pos)) board_empty all_pos

(*tail recursive*)
let board_rotated (board: board_t) cnt current=
...
let board_encode (board: board_t) =
    Pervasives.min ((board_rotated board) @ (board_rotated (board_flipped board)))

let board_move_white
(*init board*)
let board_init () =
    board_move_black
        (board_move_black
            (board_move_white
                (board_move_white board_empty (4, 4))
                (5, 5))
            (4, 5))
        (5, 4)
  ...
  match com with
    ...
    | Mv (i,j) ->
        let ms = flippable_indices board (i,j) in
            List.fold_left (fun bb (ii, jj) -> board_move_black bb (ii, jj)) board ms

let valid_moves board =
  List.filter (is_valid_move board) all_pos

let board_move board pos =
